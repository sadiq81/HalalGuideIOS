//
//  HGFilterLocationViewController.m
//  HalalGuide
//
...
@property(strong, nonatomic) HGFilterSwitchView *switchView;
...
@implementation HGFilterLocationViewController {
...
- (void)setupViews {
    ...
    self.dismiss = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"HGFilterLocationViewController.button.dismiss", nil) style:UIBarButtonItemStylePlain block:nil];
    ...
    self.distanceHeadline.text = NSLocalizedString(@"HGFilterLocationViewController.label.max.distance", nil);
    ...
    self.switchView = [[HGFilterSwitchView alloc] initWithViewModel:self.viewModel];
...
- (void)setDistanceLabelText {
        if (self.distanceSlider.value != 20) {
            ...
            self.distanceLabel.text = NSLocalizedString(@"HGFilterLocationViewController.label.unlimited", nil);
        }
}
