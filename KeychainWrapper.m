...
+ (NSMutableDictionary *)setupSearchDirectoryForIdentifier:(NSString *)identifier {
    ...
    [searchDictionary setObject:@"Halalguide" forKey:(__bridge id)kSecAttrService];
...
+ (BOOL)compareKeychainValueForMatchingPIN:(NSUInteger)pinHash
{
    ...
    if ([[self keychainStringFromMatchingIdentifier:@"pin"] isEqualToString:[self securedSHA256DigestHashForPIN:pinHash]]) {
        return YES;
    } else {
        return NO;
    }
...
+ (NSString *)securedSHA256DigestHashForPIN:(NSUInteger)pinHash
{
    ...
    // 2
    NSString *computedHashString = [NSString stringWithFormat:@"%@%lu%@", name, (unsigned long)pinHash, @"UoRXE2J0egWMUIrTJlKMCrCADmXDnbKMbz18Qk2vPId6P3UVMgKKVNriRIWKY0hvu4MzX7Z0RaDh8TTw"];
    // 3
